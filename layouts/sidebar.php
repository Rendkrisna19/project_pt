...
// Mengambil data dari session, dengan nilai default jika tidak ada
$currentPage = $currentPage ?? 'dashboard';
$userName    = $_SESSION['user_nama'] ?? 'Pengguna';
// !! PENTING: Tambahkan variabel untuk role dari session
// Asumsikan role disimpan di $_SESSION['user_role'] setelah login
// Nilai defaultnya 'admin' jika tidak ada session role
...
<aside class="fixed inset-y-0 left-0 z-50 w-72 bg-white shadow-xl md:hidden flex flex-col transform transition-transform duration-300" :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" role="dialog" aria-modal="true" aria-label="Menu navigasi">
  ...
  <nav class="px-3 py-4 overflow-y-auto flex-1">
    <span class="text-[11px] tracking-wide text-gray-400 uppercase font-semibold px-2">Menu Utama</span>
    <ul class="mt-2 space-y-1">
      <li><a href="index.php" class="flex items-center p-3 rounded-lg transition-colors duration-200 <?= activeClass('dashboard',$currentPage) ?>" @click="sidebarOpen=false"><i data-lucide="layout-dashboard" class="w-5 h-5 mr-3"></i><span class="font-medium">Dashboard</span></a></li>
      <li><a href="pemeliharaan.php" class="flex items-center p-3 rounded-lg transition-colors duration-200 <?= activeClass('pemeliharaan',$currentPage) ?>" @click="sidebarOpen=false"><i data-lucide="gauge" class="w-5 h-5 mr-3"></i><span class="font-medium">Pemeliharaan</span></a></li>
