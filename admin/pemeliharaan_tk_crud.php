<?php
// pages/pemeliharaan_tk  _crud.php — LIST mendukung semua AFD (opsional filter AFD), CRUD + SweetAlert handled on client
session_start();
...
if ($ket!==''){ $where.=" AND ket LIKE :k"; $p[':k']="%$ket%"; }
  $sql="SELECT * FROM pemeliharaan_tk  $where ORDER BY jenis_nama ASC, unit_kode ASC, id ASC";
  $st=$pdo->prepare($sql);
  ...
  $jenisMaster = $pdo->query("SELECT nama FROM md_pemeliharaan_tk  ORDER BY nama")->fetchAll(PDO::FETCH_COLUMN);
...
try{
  if ($act==='store' || $act==='update'){
    ...
    $jenis_nm = $jenis_id ? getNamaById($pdo,'md_pemeliharaan_tk  ',$jenis_id,'nama') : null;
    ...
    if ($act==='store'){
      $sql="INSERT INTO pemeliharaan_tk 
      ...
      $sql="UPDATE pemeliharaan_tk   SET
  ...
  if ($act==='delete'){
    $id=(int)($_POST['id']??0);
    if ($id<=0) out(false,'ID tidak valid');
    $pdo->prepare("DELETE FROM pemeliharaan_tk   WHERE id=:id")->execute([':id'=>$id]);
