<?php
// pages/pemeliharaan_mn.php — Rekap PN (Sticky Header & Realtime Filter)
...
// MODIFIKASI: HK ditangkap sebagai string/int ID agar match dengan dropdown
...
function exportData(type) {
        ...
        const baseUrl = 'cetak/'; 
        const file = type === 'pdf' ? 'pemeliharaan_mn_pdf.php' : 'pemeliharaan_mn_excel.php';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    let abortController = null; // Untuk membatalkan request sebelumnya jika user mengetik cepat
    ...
    async function loadData() {
        ...
        const params = new URLSearchParams({
            ...
            jenis: fJenis.value,
            hk: fHk.value, // Sekarang mengirim ID dari select option
            stood_id: fStood.value,
            ket: fKet.value
        });

        // Update URL Browser tanpa reload (agar kalau direfresh filter tetap ada)
        const newUrl = `${window.location.pathname}?${params.toString()}`;
        window.history.pushState({path: newUrl}, '', newUrl);

        try {
            const res = await fetch('pemeliharaan_mn_crud.php?' + params.toString(), { 
            ...
            if (err.name === 'AbortError') return; // Abaikan error abort
            tbody.classList.remove('table-loading');
            loadingInd.classList.add('hidden');
    ...
    function renderTable(rows, kebunMap, stoodMap, stoodOrder) {
        // Urutan group Stood dari master atau default
    ...
    // 1. Debounce function untuk input text (biar gak spam request)
    ...
    // 2. Attach Listeners
    // MODIFIKASI: fHk dipindahkan ke sini karena sekarang dia Select Box
    [fKebun, fJenis, fStood, fHk].forEach(el => {
        el.addEventListener('change', loadData);
    });
    // Untuk Input Text/Number -> Pakai Debounce
    ...
    btnReset.addEventListener('click', () => {
        ...
        fKebun.value = "0";
        fJenis.value = "";
        fHk.value = ""; // Value dropdown default
        fStood.value = "0";
        fKet.value = "";
        loadData();
    });

    // 4. Attach Edit/Delete Button Events (Called after render)
          ...
          b.addEventListener('click', async () => {
            ...
            try {
                ...
                if (jj && jj.success) {
                    ...
                    loadData(); // Reload table aja, gak perlu reload page
                } else {
    ...
    /* ===== Modal (Create/Update) Tetap Sama, cuma onsubmit reload table ===== */
    let MODAL = null;
    function modalTpl() {
      // (Template HTML Modal sama persis seperti sebelumnya, tidak diubah)
      return `
<div id="pn-modal" class="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4 backdrop-blur-sm transition-opacity">
  <div class="bg-white rounded-2xl w-full max-w-5xl shadow-2xl transform transition-all scale-100">
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50 rounded-t-2xl">
      <h3 id="pn-title" class="font-bold text-xl text-gray-800">Form MN</h3>
