  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const ENTITIES = {
      ...
      fisik: {
        ...
        table: ['Nama', 'Aksi'],
        fields: [
          { name: 'nama', label: 'Nama Fisik (Ha/Pkk/Unit/..)', type: 'text', required: true }
        ]
      },
      satuan: {
        title: 'Satuan',
        table: ['Nama', 'Aksi'],
        fields: [
          { name: 'nama', label: 'Nama Satuan (Kg/Liter/..)', type: 'text', required: true }
        ]
      },
      tenaga: {
        title: 'Tenaga',
      ...
      rayon: {
        ...
        fields: [
          { name: 'nama', label: 'Nama Rayon', type: 'text', required: true }
        ]
      },
      apl: {
        title: 'APL',
        table: ['Nama APL', 'Aksi'],
        fields: [
          { name: 'nama', label: 'Nama APL', type: 'text', required: true }
        ]
      },
      keterangan: {
      ...
      return fetch('master_data_crud.php', { method: 'POST', body: fd })
        ...
        .then(j => {
          ...
          if (j && j.success && typeof j.total === 'number') {
            ...
            updatePageInfo();
            renderPager();
            clientCache = { entity: null, rows: [] }; // Clear client cache if server paginates
            return true;
          }
            ...
            fetch('master_data_crud.php', { method: 'POST', body: fd })
              ...
              .then(j => {
                if (j.success) {
                  ...
                  clientCache = { entity: null, rows: [] }; // Invalidate cache
...
<style>
  #tabs .tab {
    ...
    color: #334155; /* gray-700 */ background: #f0fdf4; /* green-50 */
    transition: background-color .15s ease, color .15s ease, border-color .15s ease;
    white-space: nowrap;
  }
  #tabs .tab:hover { background: #dcfce7; /* green-100 */ color: #166534; /* green-800 */ }
  #tabs .tab.active {
    background: #22c55e; /* green-500 */ border-color: #16a34a; /* green-600 */
    color: white; font-weight: 600;
  }
  .icon-btn {
    display: inline-flex; align-items: center; justify-content: center;
    width: 2rem; height: 2rem; border-radius: .375rem; /* rounded-md */
    background: transparent; border: none; cursor: pointer;
    transition: background-color .15s ease;
  }
   .icon-btn:hover { background-color: rgba(0,0,0,0.05); }
   .icon-btn:active { transform: scale(.98); }
   table thead th { position: sticky; top: 0; background: #f9fafb; z-index: 10;} /* bg-gray-50 */
   /* Styling for required field indicator */
    input:required, select:required, textarea:required { border-left: 3px solid #f87171; /* red-400 */}
    input:invalid, select:invalid, textarea:invalid { border-color: #f87171; } /* For native validation */
</style>