...
//                        + NEW: Bibit TM (md_jenis_bibittm) & Bibit PN (md_jenis_bibitpn)
//                        + checkbox support for is_active)
...
<div class="space-y-6">
  ...
  <div class="bg-white p-2 md:p-3 rounded-xl shadow-sm overflow-x-auto">
    <div id="tabs" class="flex gap-2 md:gap-3 whitespace-nowrap">
      ...
      <button data-entity="bibit_tm" class="tab">Bibit MN</button>
      <button data-entity="bibit_pn" class="tab">Bibit PN</button>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const ENTITIES = {
    ...
    jenis_pekerjaan_mingguan: { title:'Jenis Pekerjaan (Mingguan)', table:['Nama','Keterangan','Aksi'], fields:[
      {name:'nama',       label:'Nama',       type:'text', required:true},
      {name:'keterangan', label:'Keterangan', type:'text'}
    ]},
    ...
    // ============== NEW: BIBIT TM & BIBIT PN ==============
    // Tabel target: md_jenis_bibittm & md_jenis_bibitpn
    // Kolom yang diharapkan: id, kode (nullable), nama (required), is_active (tinyint 0/1), created_at/updated_at (opsional)
    bibit_tm: {
      title: 'Bibit TM',
      table: ['Kode', 'Nama Bibit TM', 'Aktif', 'Aksi'],
      fields: [
        { name:'kode',      label:'Kode (opsional)', type:'text' },
        { name:'nama',      label:'Nama Bibit TM',   type:'text', required:true },
        { name:'is_active', label:'Aktif?',          type:'checkbox' } // boolean
      ]
    },
    bibit_pn: {
      title: 'Bibit PN',
      table: ['Kode', 'Nama Bibit PN', 'Aktif', 'Aksi'],
      fields: [
        { name:'kode',      label:'Kode (opsional)', type:'text' },
    ...
    sap: { title:'No SAP', table:['No SAP','Deskripsi','Aksi'], fields:[
      {name:'no_sap',    label:'No SAP',    type:'text', required:true},
    ...
    kode_aktivitas:{ title:'Kode Aktivitas', table:['Kode','Nama','No SAP','Aksi'], fields:[
      {name:'kode',      label:'Kode', type:'text', required:true},
      {name:'nama',      label:'Nama', type:'text', required:true},
      {name:'no_sap_id', label:'No SAP', type:'select', options:OPTIONS_SAP}
    ]},
    anggaran:{ title:'Anggaran', table:['Periode','Unit','Kode Aktivitas','Pupuk','Angg Bulan','Angg Tahun','Aksi'], fields:[
      ...
      {name:'anggaran_bulan_ini', label:'Anggaran Bulan Ini', type:'number', step:'0.01', required:true, min:0},
      {name:'anggaran_tahun',     label:'Anggaran Tahun',     type:'number', step:'0.01', required:true, min:0}
    ]},

    // Tambahan master ringkas
    ...
    // Pemeliharaan master (nama + deskripsi)
  ...
  // ====== inputEl: support select, textarea, number/text, and NEW: checkbox ======
  function inputEl(f) {
    ...
    if (f.type === 'select') {
      ...
      // checkbox tampil tanpa span col 2
      control = `<label class="inline-flex items-center gap-2">
        <input type="checkbox" id="${f.name}" name="${f.name}" class="h-4 w-4 rounded border-gray-300">
  ...
  function renderForm(entity, data = {}) {
    ...
    ENTITIES[entity].fields.forEach(f => {
      ...
      if (f.type === 'checkbox') {
        inputElement.checked = String(val ?? '1') === '1'; // default aktif bila null
      } else {
  ...
  function rowCols(entity, r){
    switch(entity){
      case 'kebun': return `<td class="py-2.5 px-3">${cell(r.kode)}</td><td class="py-2.5 px-3">${cell(r.nama_kebun)}</td><td class="py-2.5 px-3">${cell(r.keterangan)}</td>`;
      case 'bahan_kimia': return `<td class="py-2.5 px-3">${cell(r.kode)}</td><td class="py-2.5 px-3">${cell(r.nama_bahan)}</td><td class="py-2.5 px-3">${cell(r.nama_satuan||'')}</td><td class="py-2.5 px-3">${cell(r.keterangan)}</td>`;
      case 'jenis_pekerjaan_mingguan': return `<td class="py-2.5 px-3">${cell(r.nama)}</td><td class="py-2.5 px-3">${cell(r.keterangan)}</td>`;
      case 'jenis_pekerjaan':          return `<td class="py-2.5 px-3">${cell(r.nama)}</td><td class="py-2.5 px-3">${cell(r.keterangan)}</td>`;

      // NEW: Bibit TM & Bibit PN
      case 'bibit_tm':
      case 'bibit_pn': {
        const aktifBadge = String(r.is_active ?? '1') === '1'
          ? '<span class="px-2 py-0.5 rounded text-xs bg-emerald-100 text-emerald-700">Aktif</span>'
          : '<span class="px-2 py-0.5 rounded text-xs bg-gray-200 text-gray-700">Nonaktif</span>';
        return `<td class="py-2.5 px-3">${cell(r.kode)}</td><td class="py-2.5 px-3">${cell(r.nama)}</td><td class="py-2.5 px-3">${aktifBadge}</td>`;
      }
      ...
      case 'rayon': return `<td class="py-2.5 px-3">${cell(r.nama)}</td>`;
      case 'apl':   return `<td class="py-2.5 px-3">${cell(r.nama)}</td>`;
      case 'keterangan': return `<td class="py-2.5 px-3">${cell(r.keterangan)}</td>`;
      case 'asal_gudang': return `<td class="py-2.5 px-3">${cell(r.nama)}</td>`;

      case 'pem_tm': case 'pem_tu': case 'pem_tk': case 'pem_tbm1': case 'pem_tbm2': case 'pem_tbm3': case 'pem_pn': case 'pem_mn':
        return `<td class="py-2.5 px-3">${cell(r.nama)}</td><td class="py-2.5 px-3">${cell(r.deskripsi)}</td>`;
    }
    return '';
  }
  ...
  function renderPager(){
    ...
    const makeBtn = (label, targetPage, disabled=false, active=false) => {
      ...
      a.innerHTML = label; a.className = 'px-3 py-1 border rounded text-sm';
      if (active) { a.className += ' bg-emerald-600 text-white border-emerald-600 cursor-default'; }
      else if (!disabled){ a.className += ' hover:bg-gray-100'; }
      if (disabled){ a.disabled = true; a.classList.add('opacity-40','cursor-not-allowed'); }
    ...
    if (start > 1){ makeBtn('1',1); if (start > 2) pager.insertAdjacentHTML('beforeend','<span class="px-2">...</span>'); }
    for (let p=start; p<=end; p++) makeBtn(String(p), p, false, p===page);
    if (end < totalPages){ if (end < totalPages-1) pager.insertAdjacentHTML('beforeend','<span class="px-2">...</span>'); makeBtn(String(totalPages), totalPages); }
  ...
  function renderHeadAndLoad(entity){
    ...
    loadServer(entity).catch(err => {
      ...
      $('#page-info').textContent = 'Gagal memuat'; $('#pager').innerHTML = '';
    });
  }
  ...
  document.getElementById('crud-form').addEventListener('submit', e => {
    ...
    // Normalisasi checkbox → 1/0
    def.fields.forEach(f=>{
      if (f.type === 'checkbox'){
        const el = formElement.querySelector(`[name="${f.name}"]`);
        fd.set(f.name, el && el.checked ? '1' : '0');
      }
    });
    ...
    def.fields.forEach(f=>{
      ...
      if (f.required && !(input.value||'').trim()){
        ok = false; input.classList.add('border-red-500');
      } else { input.classList.remove('border-red-500'); }
    });
    if (!ok){ Swal.fire('Oops','Lengkapi semua field wajib (*)','warning'); return; }
    ...
    fetch('master_data_crud.php',{method:'POST', body:fd})
      ...
      .then(j=>{
        if (j.success){
          ...
          close(); clientCache = { entity:null, rows:[] }; page=1; renderHeadAndLoad(entity);
        } else {
          const msg = j.errors ? j.errors.join('<br>') : (j.message||'Terjadi kesalahan.');
...
<style>
  #tabs .tab{ padding:.5rem .75rem; border-radius:.5rem; border:1px solid transparent; color:#334155; background:#f0fdf4; transition:background-color .15s ease,color .15s ease,border-color .15s ease; white-space:nowrap; }
  #tabs .tab:hover{ background:#dcfce7; color:#166534; }
  #tabs .tab.active{ background:#22c55e; border-color:#16a34a; color:white; font-weight:600; }

  .icon-btn{ display:inline-flex; align-items:center; justify-content:center; width:2rem; height:2rem; border-radius:.375rem; background:transparent; border:none; cursor:pointer; transition:background-color .15s ease; }
  .icon-btn:hover{ background-color:rgba(0,0,0,.05); }
  .icon-btn:active{ transform:scale(.98); }

  table thead th{ position:sticky; top:0; background:#f9fafb; z-index:10; }
  input:required, select:required, textarea:required{ border-left:3px solid #f87171; }
