<?php
  // pages/pemeliharaan_tbm1.php — Rekap TM (grup per Jenis, subtotal RY A/RY B, Grand Total)
  ...
  $jenisMaster = $conn->query("SELECT nama FROM md_pemeliharaan_tbm1 ORDER BY nama")->fetchAll(PDO::FETCH_COLUMN);
  ...
  <div class="space-y-6">
    <div>
      <h1 class="text-3xl font-bold">Pemeliharaan TBM I</h1>
    ...
    <form method="GET" class="bg-white p-4 rounded-xl shadow toolbar">
      ...
      <div class="md-span-2 flex items-end gap-2">
        ...
        <a href="pemeliharaan_tbm1.php" class="btn btn-gray"><i class="ti ti-refresh"></i> Reset</a>
      </div>
    </form>
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    (async function loadAll(){
      ...
      const res = await fetch('pemeliharaan_tbm1_crud.php?'+qs.toString(), {credentials:'same-origin'});
      ...
      if (!IS_STAF){
        ...
        tbody.querySelectorAll('[data-del]').forEach(b=>{
          b.addEventListener('click', async ()=>{
            ...
            const r = await fetch('pemeliharaan_tbm1_crud.php',{method:'POST',body:fd});
      ...
      <form id="tm-form" class="p-4 grid grid-cols-12 gap-3">
        ...
        <div class="col-span-5"><label class="text-xs font-semibold">Jenis Pekerjaan</label>
          <select name="jenis_id" class="w-full border rounded px-3 py-2" required>
            <option value="">— Pilih —</option>
            <?php foreach($conn->query("SELECT id,nama FROM md_pemeliharaan_tbm1 ORDER BY nama")->fetchAll(PDO::FETCH_ASSOC) as $x){ echo '<option value="'.$x['id'].'">'.htmlspecialchars($x['nama']).'</option>'; } ?>
          </select>
        </div>
