<?php
  // pages/pemeliharaan_tu.php — Rekap TM (grup per Jenis, subtotal RY A/RY B, Grand Total)
  ...
  <div class="space-y-6">
    <div>
      <h1 class="text-3xl font-bold">Pemeliharaan TU</h1>
    ...
    <form method="GET" class="bg-white p-4 rounded-xl shadow toolbar">
      ...
      <div class="md-span-2 flex items-end gap-2">
        ...
        <a href="pemeliharaan_tu.php" class="btn btn-gray"><i class="ti ti-refresh"></i> Reset</a>
      </div>
    </form>
      ...
      <form id="tm-form" class="p-4 grid grid-cols-12 gap-3">
        ...
        <div class="col-span-5"><label class="text-xs font-semibold">Jenis Pekerjaan</label>
          <select name="jenis_id" class="w-full border rounded px-3 py-2" required>
            <option value="">— Pilih —</option>
            <?php foreach($conn->query("SELECT id,nama FROM md_pemeliharaan_tu ORDER BY nama")->fetchAll(PDO::FETCH_ASSOC) as $x){ echo '<option value="'.$x['id'].'">'.htmlspecialchars($x['nama']).'</option>'; } ?>
          </select>
        </div>
