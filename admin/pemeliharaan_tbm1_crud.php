<?php
// pages/pemeliharaan_tbm1  _crud.php — LIST mendukung semua AFD (opsional filter AFD), CRUD + SweetAlert handled on client
session_start();
...
if (($_GET['action'] ?? '') === 'list') {
  ...
  $sql="SELECT * FROM pemeliharaan_tbm1  $where ORDER BY jenis_nama ASC, unit_kode ASC, id ASC";
  $st=$pdo->prepare($sql);
  ...
  $jenisMaster = $pdo->query("SELECT nama FROM md_pemeliharaan_tbm1  ORDER BY nama")->fetchAll(PDO::FETCH_COLUMN);
...
try{
  if ($act==='store' || $act==='update'){
    ...
    $jenis_nm = $jenis_id ? getNamaById($pdo,'md_pemeliharaan_tbm1  ',$jenis_id,'nama') : null;
    ...
    if ($act==='store'){
      $sql="INSERT INTO pemeliharaan_tbm1 
      ...
      $sql="UPDATE pemeliharaan_tbm1   SET
  ...
  if ($act==='delete'){
    $id=(int)($_POST['id']??0);
    if ($id<=0) out(false,'ID tidak valid');
    $pdo->prepare("DELETE FROM pemeliharaan_tbm1   WHERE id=:id")->execute([':id'=>$id]);
