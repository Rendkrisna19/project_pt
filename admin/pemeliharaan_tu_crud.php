<?php
// pages/pemeliharaan_tu_crud.php — LIST mendukung semua AFD (opsional filter AFD), CRUD + SweetAlert handled on client
session_start();
...
if (($_GET['action'] ?? '') === 'list') {
  ...
  $sql="SELECT * FROM pemeliharaan_tu $where ORDER BY jenis_nama ASC, unit_kode ASC, id ASC";
  $st=$pdo->prepare($sql);
  ...
  $jenisMaster = $pdo->query("SELECT nama FROM md_pemeliharaan_tu ORDER BY nama")->fetchAll(PDO::FETCH_COLUMN);
...
try{
  if ($act==='store' || $act==='update'){
    ...
    $jenis_nm = $jenis_id ? getNamaById($pdo,'md_pemeliharaan_tu',$jenis_id,'nama') : null;
    ...
    if ($act==='store'){
      $sql="INSERT INTO pemeliharaan_tu
      ...
      $sql="UPDATE pemeliharaan_tu SET
  ...
  if ($act==='delete'){
    $id=(int)($_POST['id']??0);
    if ($id<=0) out(false,'ID tidak valid');
    $pdo->prepare("DELETE FROM pemeliharaan_tu WHERE id=:id")->execute([':id'=>$id]);
