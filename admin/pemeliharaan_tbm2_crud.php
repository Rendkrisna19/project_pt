<?php
// pages/pemeliharaan_tbm2  _crud.php — LIST mendukung semua AFD (opsional filter AFD), CRUD + SweetAlert handled on client
session_start();
...
if ($ket!==''){ $where.=" AND ket LIKE :k"; $p[':k']="%$ket%"; }
  $sql="SELECT * FROM pemeliharaan_tbm2  $where ORDER BY jenis_nama ASC, unit_kode ASC, id ASC";
  $st=$pdo->prepare($sql);
  ...
  $jenisMaster = $pdo->query("SELECT nama FROM md_pemeliharaan_tbm2  ORDER BY nama")->fetchAll(PDO::FETCH_COLUMN);
...
try{
  if ($act==='store' || $act==='update'){
    ...
    $jenis_nm = $jenis_id ? getNamaById($pdo,'md_pemeliharaan_tbm2  ',$jenis_id,'nama') : null;
    ...
    if ($act==='store'){
      $sql="INSERT INTO pemeliharaan_tbm2 
      ...
      $sql="UPDATE pemeliharaan_tbm2   SET
  ...
  if ($act==='delete'){
    $id=(int)($_POST['id']??0);
    if ($id<=0) out(false,'ID tidak valid');
    $pdo->prepare("DELETE FROM pemeliharaan_tbm2   WHERE id=:id")->execute([':id'=>$id]);
