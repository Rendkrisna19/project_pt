...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    ...
    <?php if (!$isStaf): // MODIFIKASI: Tombol hanya tampil jika bukan staf ?>
      ...
      <table class="min-w-full text-sm">
        <thead class="bg-green-500 sticky top-0 z-10">
          <tr class="text-gray-100">
            ...
            <?php if (!$isStaf): // MODIFIKASI: Kolom hanya tampil jika bukan staf ?>
...
<?php if (!$isStaf): // MODIFIKASI: Modal hanya ada jika bukan staf ?>
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  function renderPage(){
    ...
    if(rows.length===0){
      ...
      tbody.innerHTML = rows.map(x=>{
        ...
        // MODIFIKASI: Sel aksi hanya dibuat jika bukan staf
        let actionCell = '';
        if (!IS_STAF) {
            actionCell = `
            <td class="py-2 px-3">
  ...
  // Event untuk export tetap di luar agar bisa diakses semua role
  document.getElementById('btn-export-excel').addEventListener('click', () => {
    const qs=new URLSearchParams({
      csrf_token:'<?= htmlspecialchars($CSRF) ?>',kebun_id:$('#f-kebun').value||'',
      unit_id:$('#f-unit').value||'',bulan:$('#f-bulan').value||'',
      tahun:$('#f-tahun').value||''
    }).toString();
