<?php
// pages/pemeliharaan_tm.php — Rekap TM (Live Filter / No Reload)
...
$userRole = $_SESSION['user_role'] ?? 'viewer'; // Default ke viewer jika error session
...
$jenisMaster = $conn->query("SELECT nama FROM md_pemeliharaan_tm ORDER BY nama")->fetchAll(PDO::FETCH_COLUMN);
$TENAGA      = $conn->query("SELECT id, kode FROM md_tenaga ORDER BY kode")->fetchAll(PDO::FETCH_ASSOC);
$JENIS_ROWS  = $conn->query("SELECT id, nama FROM md_pemeliharaan_tm ORDER BY nama")->fetchAll(PDO::FETCH_ASSOC);
...
<script>
   ...
   function exportData(type) {
    ...
    const baseUrl = 'cetak/'; 
    const file = type === 'pdf' ? 'pemeliharaan_tm_pdf.php' : 'pemeliharaan_tm_excel.php';
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  async function fetchData() {
    ...
    try {
        const res = await fetch('pemeliharaan_tm_crud.php?' + qs.toString(), {credentials:'same-origin'});
  ...
  function renderTable(data) {
    ...
    for (const jn of order){
      html += `<tr class="group-head"><td colspan="${COLS_TOTAL}"><i class="ti ti-folder-open text-xm mr-1 align-text-bottom"></i> ${jn}</td></tr>`;
  ...
  function bindActions(){
      ...
      tbody.querySelectorAll('[data-del]').forEach(b=>{
        b.addEventListener('click', async ()=>{
          ...
          fd.append('id',id);
          const r = await fetch('pemeliharaan_tm_crud.php',{method:'POST',body:fd});
...
<div id="tm-modal" class="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-3 sm:p-4 backdrop-blur-sm transition-opacity">
  <div class="bg-white rounded-xl sm:rounded-2xl w-full max-w-5xl shadow-2xl transform transition-all scale-100 flex flex-col max-h-[95vh] sm:max-h-[90vh]">
    ...
    <div class="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 bg-gray-50 rounded-t-xl sm:rounded-t-2xl shrink-0">
      <h3 id="tm-title" class="font-bold text-lg sm:text-xl text-gray-800">Form TM</h3>
