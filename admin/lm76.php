...
<div class="space-y-6">
  ...
  <div>
    ...
    <p class="text-gray-500">Input per afdeling (akan dipakai untuk template Excel LM-76)</p>
  </div>
  ...
  <div class="bg-white rounded-xl shadow-sm overflow-x-auto">
    <table class="min-w-full text-sm">
      ...
      <tbody id="tbody-data"><tr><td colspan="13" class="text-center py-10 text-gray-500">Memuat…</td></tr></tbody>
    </table>
  </div>
</div>
  ...
  <div class="bg-white p-6 md:p-8 rounded-xl shadow-xl w-full max-w-5xl">
    ...
    <form id="crud-form" novalidate>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div><label class="block text-sm mb-1">T.T</label><input type="text" id="tt" name="tt" class="w-full border rounded px-3 py-2"></div>
        <div><label class="block text-sm mb-1">Blok</label><input type="text" id="blok" name="blok" class="w-full border rounded px-3 py-2"></div>
        ...
        <div><label class="block text-sm mb-1">Varietas</label><input type="text" id="varietas" name="varietas" class="w-full border rounded px-3 py-2"></div>
        <div><label class="block text-sm mb-1">Prod BI Real</label><input type="number" step="0.01" id="prod_bi_realisasi" name="prod_bi_realisasi" class="w-full border rounded px-3 py-2"></div>
        <div><label class="block text-sm mb-1">Prod BI Angg</label><input type="number" step="0.01" id="prod_bi_anggaran" name="prod_bi_anggaran" class="w-full border rounded px-3 py-2"></div>
        <div><label class="block text-sm mb-1">Prod SD Real</label><input type="number" step="0.01" id="prod_sd_realisasi" name="prod_sd_realisasi" class="w-full border rounded px-3 py-2"></div>

        <div><label class="block text-sm mb-1">Prod SD Angg</label><input type="number" step="0.01" id="prod_sd_anggaran" name="prod_sd_anggaran" class="w-full border rounded px-3 py-2"></div>
        <div><label class="block text-sm mb-1">Jml Tandan BI</label><input type="number" id="jumlah_tandan_bi" name="jumlah_tandan_bi" class="w-full border rounded px-3 py-2"></div>
        <div><label class="block text-sm mb-1">PSTB BI (Ton/Ha)</label><input type="number" step="0.01" id="pstb_ton_ha_bi" name="pstb_ton_ha_bi" class="w-full border rounded px-3 py-2"></div>
        <div><label class="block text-sm mb-1">PSTB TL (Ton/Ha)</label><input type="number" step="0.01" id="pstb_ton_ha_tl" name="pstb_ton_ha_tl" class="w-full border rounded px-3 py-2"></div>

        <div><label class="block text-sm mb-1">Panen HK Realisasi</label><input type="number" step="0.01" id="panen_hk_realisasi" name="panen_hk_realisasi" class="w-full border rounded px-3 py-2"></div>
        <div><label class="block text-sm mb-1">Panen Ha BI</label><input type="number" step="0.01" id="panen_ha_bi" name="panen_ha_bi" class="w-full border rounded px-3 py-2"></div>
        <div><label class="block text-sm mb-1">Panen Ha SD</label><input type="number" step="0.01" id="panen_ha_sd" name="panen_ha_sd" class="w-full border rounded px-3 py-2"></div>
        <div><label class="block text-sm mb-1">Freq Panen BI</label><input type="number" id="frek_panen_bi" name="frek_panen_bi" class="w-full border rounded px-3 py-2"></div>

        <div><label class="block text-sm mb-1">Freq Panen SD</label><input type="number" id="frek_panen_sd" name="frek_panen_sd" class="w-full border rounded px-3 py-2"></div>
      </div>
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  const modal = $('#crud-modal'); const open=()=>{modal.classList.remove('hidden');modal.classList.add('flex')}; const close=()=>{modal.classList.add('hidden');modal.classList.remove('flex')};
  $('#btn-add').addEventListener('click',()=>{document.getElementById('crud-form').reset(); document.getElementById('form-action').value='store'; document.getElementById('form-id').value=''; open();});
  $('#btn-close').addEventListener('click',close); $('#btn-cancel').addEventListener('click',close);

  function refresh(){
    const fd=new FormData(); fd.append('csrf_token','<?= htmlspecialchars($CSRF) ?>'); fd.append('action','list'); fd.append('unit_id', selU.value); fd.append('bulan', selB.value); fd.append('tahun', selT.value);
    fetch('lm76_crud.php',{method:'POST',body:fd}).then(r=>r.json()).then(j=>{
      if(!j.success){tbody.innerHTML=`<tr><td colspan="13" class="text-center py-8 text-red-500">${j.message||'Error'}</td></tr>`;return;}
      if(!j.data.length){tbody.innerHTML=`<tr><td colspan="13" class="text-center py-8 text-gray-500">Belum ada data.</td></tr>`;return;}
  ...
  document.body.addEventListener('click',e=>{
    if(e.target.classList.contains('btn-edit')){
      ...
      form.reset(); document.getElementById('form-action').value='update'; document.getElementById('form-id').value=d.id;
...
// === Export Excel & PDF (bawa filter aktif + CSRF) ===
