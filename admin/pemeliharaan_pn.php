<?php
// pages/pemeliharaan_pn.php — Rekap PN per STOOD (grup + subtotal), tanpa kolom "Stood"
// Versi: 2025-11-10 — per Stood (master md_jenis_bibitpn), filter & form pakai dropdown Stood
// MODIFIKASI: 2025-11-11 — Pindah kolom "Jumlah" (total realisasi) setelah bulan, sebelum +/-
// CATATAN: Filter 'ket' diproses di pemeliharaan_pn_crud.php, BUKAN DI FILE INI.
...
<div class="space-y-6">
  <div>
    <h1 class="text-3xl font-bold">Pemeliharaan PN</h1>
  </div>
  ...
  <div class="bg-white rounded-xl shadow freeze-parent">
    <div class="px-3 py-2 border-b flex items-center gap-3">
      <div class="font-semibold text-gray-700 flex-1">
        Rekap PN • Tahun <?= htmlspecialchars((string)$f_tahun) ?>
      </div>
      <?php if (!$isStaf): ?>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    (async function loadAll() {
      ...
      try {
        ...
        const res = await fetch('pemeliharaan_pn_crud.php?' + qs.toString(), {
        ...
        if (!IS_STAF) {
          ...
          tbody.querySelectorAll('[data-del]').forEach(b => {
            b.addEventListener('click', async () => {
              ...
              fd.append('id', id);
              const r = await fetch('pemeliharaan_pn_crud.php', {
                method: 'POST',
                body: fd
              });
