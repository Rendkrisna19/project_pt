<?php
// pages/pemeliharaan_pn.php — Rekap PN (tanpa Jumlah/Total), ada Kebun & Stood
session_start();
...
<div class="space-y-6">
  <div><h1 class="text-3xl font-bold">Pemeliharaan PN</h1></div>
  ...
  <form method="GET" class="bg-white p-4 rounded-xl shadow toolbar">
    ...
    <div class="md-span-2 flex items-end gap-2">
      ...
      <a href="pemeliharaan_pn.php" class="btn btn-gray"><i class="ti ti-refresh"></i> Reset</a>
    </div>
  </form>
  ...
  <div class="bg-white rounded-xl shadow freeze-parent">
    <div class="px-3 py-2 border-b flex items-center gap-3">
      <div class="font-semibold text-gray-700 flex-1">
        Rekap PN • Tahun <?= htmlspecialchars($f_tahun) ?> <?= $f_afd ? "• $f_afd" : '' ?>
      </div>
      <?php if(!$isStaf): ?>
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  (async function loadAll(){
    ...
    const res = await fetch('pemeliharaan_pn_crud.php?'+qs.toString(), {credentials:'same-origin'});
...
<div id="pn-modal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
  <div class="bg-white rounded-xl w-full max-w-5xl shadow-xl">
    <div class="flex items-center justify-between p-4 border-b">
      ...
      <button id="pn-x" class="text-xl">&times;</button>
    </div>
    <form id="pn-form" class="p-4 grid grid-cols-12 gap-3">
      ...
      <div class="col-span-6"><label class="text-xs font-semibold">Jenis Pekerjaan</label>
        <select name="jenis_id" class="w-full border rounded px-3 py-2" required>
          <option value="">— Pilih —</option>
          <?php foreach($conn->query("SELECT id,nama FROM md_pemeliharaan_pn ORDER BY nama")->fetchAll(PDO::FETCH_ASSOC) as $x){ echo '<option value="'.$x['id'].'">'.htmlspecialchars($x['nama']).'</option>'; } ?>
        </select>
      </div>
  ...
  function openForm(d={}) {
    ...
    const F=document.getElementById('pn-form'); const T=document.getElementById('pn-title');
