...
/* Staf hanya boleh CREATE */
if ($isStaf && in_array($action, ['update','delete'], true)) {
...
function validDate($d){
  if ($d === '') return false;
  $ts = strtotime($d);
  return $ts !== false && date('Y-m-d', $ts) === $d;
}
...
try{
  ...
  $kebunFromEither = function(PDO $conn, $kebun_id_input, $kebun_kode_input){
    ...
    if ($kid) {
      if (empty($kkod)) {
        $st = $conn->prepare("SELECT kode FROM md_kebun WHERE id = ?");
        $st->execute([$kid]);
        $kkod = $st->fetchColumn() ?: null;
      }
    } elseif ($kkod) {
      $st = $conn->prepare("SELECT id FROM md_kebun WHERE kode = ?");
      $st->execute([$kkod]);
      $kid = $st->fetchColumn() ?: null;
    }
    return [$kid, $kkod];
  };
  ...
  // deteksi pilihan kolom AU-58 (no_au58 / no_au_58)
  ...
  if ($action === 'store' || $action === 'create') {
    ...
    if ($tab === 'angkutan') {
      ...
      $invt      = i('invt_pokok');
      $tahunPost = i('tahun');
      $no_au58   = s('no_au_58'); // ← input AU-58
      $rayon_id      = i('rayon_id');
      $apl_id        = i('apl_id');
      $keterangan_id = i('keterangan_id');
      ...
      if ($hasNoAU58Men && $noAU58ColMen){ $cols[]=$noAU58ColMen; $vals[]=':au'; $params[':au']=($no_au58!==''?$no_au58:null); }
  ...
  error_log("Pemupukan CRUD PDO Error: " . $e->getMessage());
  echo json_encode(['success'=>false,'message'=>'Kesalahan Database: ' . $e->getMessage()]);
} catch(Throwable $e){
  error_log("Pemupukan CRUD General Error: " . $e->getMessage());
