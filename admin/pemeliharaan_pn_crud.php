<?php
// pages/pemeliharaan_pn_crud.php — LIST + CRUD PN (per STOOD master, JSON-safe)
...
if (($_GET['action'] ?? '') === 'list') {
  try {
    ...
    */
    ...
    // Master stood aktif
    $stm = $pdo->query("SELECT id, nama FROM md_jenis_bibitpn WHERE is_active=1 ORDER BY nama");
    $stood_map = [];
    $stood_order_master = [];
...
try {
  if ($act==='store' || $act==='update'){
    ...
    $stood_nm = getNamaById($pdo,'md_jenis_bibitpn',$stood_id,'nama');
    if (!$stood_nm) {
