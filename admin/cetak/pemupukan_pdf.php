...
// Cetak PDF Pemupukan (Menabur/Angkutan) – Tema Cyan, Sinkronisasi Filter Full
...
try {
    ...
    $f_tahun         = qstr($_GET['tahun'] ?? ''); // Tambahan filter tahun
    ...
    // Filter Teks Lama (legacy fallback)
    ...
    // Map kebun id->kode untuk filter
    ...
    if ($tab === 'angkutan') {
        ...
        // Logika Filter Tanggal, Bulan, Tahun
        if ($f_tanggal !== '') {
...
<style>
    ...
    /* TEMA CYAN-500 */
    .brand { background: #06b6d4; color: #fff; padding: 12px 16px; border-radius: 8px; text-align: center; margin-bottom: 10px; }
    .brand h1 { margin: 0; font-size: 18px; letter-spacing: .5px; }
    .subtitle { text-align: center; margin: 4px 0 12px; font-weight: 700; font-size: 13px; color: #0891b2; } /* Cyan 600 */
    
    table { width: 100%; border-collapse: collapse; }
    th, td { border: 1px solid #e5e7eb; padding: 6px 8px; word-wrap: break-word; }
    thead th { background: #cffafe; color: #164e63; font-weight: 700; text-align: left; } /* Cyan 100 & 900 */
    thead th.text-right { text-align: right; }
    tbody tr:nth-child(even) td { background: #f8fafc; }
    tfoot td { background: #ecfeff; font-weight: 700; color: #164e63; } /* Cyan 50 & 900 */
...
<body>
    ...
    <table>
        ...
        <tbody>
            <?php if (empty($rows)): ?>
                <tr><td colspan="<?= $tab === 'angkutan' ? 10 : 15 ?>" style="text-align:center; padding: 20px;">Belum ada data.</td></tr>
            <?php else: foreach ($rows as $r): ?>
                ...
                <?php else:
                    ...
                    $rayonDisplay = $r['rayon_nama'] ?? ($r['rayon'] ?? '-');
                    $aplDisplay   = $r['apl_nama']   ?? ($r['apl_text'] ?? '-'); 
                    $ketDisplay   = $r['keterangan_text'] ?? ($r['keterangan'] ?? '');
                ?>
                    <tr>
