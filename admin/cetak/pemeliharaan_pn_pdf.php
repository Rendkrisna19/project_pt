<?php
// pages/cetak/pemeliharaan_pn_pdf.php
// Modifikasi: Menggunakan tabel 'pemeliharaan_pn' & Logika Filter sesuai CRUD
...
try {
    ...
    // ===== 1. TANGKAP FILTER (SAMA PERSIS DENGAN CRUD) =====
    ...
    $f_hkId     = isset($_GET['hk']) && $_GET['hk'] !== '' ? (int)$_GET['hk'] : 0; // HK ditangkap sbg ID
    ...
    // Logika Filter HK (ID -> Kode) sesuai CRUD
    if ($f_hkId > 0) {
        ...
        if ($kodeHk) {
            ...
            $where .= " AND 1=0"; // Data tidak ditemukan
        }
    }
    ...
    // Filter Stood via HAVING (Sesuai CRUD)
    $having = "";
    if ($f_stoodId > 0) {
    ...
    // QUERY UTAMA (Tabel: pemeliharaan_pn)
    $sql = "SELECT 
                p.*, 
                COALESCE(NULLIF(p.stood_id,0), m.id) AS stood_id_fix,
                COALESCE(p.stood, m.nama)            AS stood_name_fix
            FROM pemeliharaan_pn p
            LEFT JOIN md_jenis_bibitpn m ON TRIM(UPPER(m.nama)) = TRIM(UPPER(p.stood))
            $where
            $having
...
<html>
<head>
    <title>Laporan Pemeliharaan PN</title>
    <style>
...
<body>
    ...
    <div class="header-box">
        <h2>REKAPITULASI PEMELIHARAAN PN</h2>
            ...
            <?php else: ?>
                ...
                ?>
                    ...
                    <tr class="sum-row">
                        ...
                        <td class="text-right"><?= $sumAnggaran > 0 ? number_format(($sumReal/$sumAnggaran)*100, 2) : '0' ?>%</td>
                    </tr>
...
$dompdf->render();
$dompdf->stream("Rekap_PN_Tahun_{$f_tahun}.pdf", ["Attachment" => false]);
?>