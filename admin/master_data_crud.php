...
/**
 * Helper: ubah '' -> null untuk kolom opsional
 */
...
try {
  ...
  if ($action === 'store' || $action === 'update') {
    ...
    // ===== Normalisasi/validasi per entity =====
    // kosong -> null untuk fleksibel kolom opsional
    foreach ($data as $k=>$v) { $data[$k] = null_if_empty($v); }

    // tahun_tanam: tahun sebagai int
    ...
    // pupuk: satuan_id optional int
    if ($entity==='pupuk') {
      $data['satuan_id'] = isset($data['satuan_id']) ? (int)$data['satuan_id'] : null;
    }

    // kode_aktivitas: no_sap_id optional int
    if ($entity==='kode_aktivitas') {
      $data['no_sap_id'] = isset($data['no_sap_id']) ? (int)$data['no_sap_id'] : null;
    }

    // anggaran: cast dan validasi
    if ($entity==='anggaran') {
      $data['tahun']               = isset($data['tahun']) ? (int)$data['tahun'] : null;
      $data['unit_id']             = isset($data['unit_id']) ? (int)$data['unit_id'] : null;
      ...
      // bulan tetap string validasi sederhana
    ...
    // Validasi minimal server-side untuk field wajib
    if (!empty($cfg['required'])) {
      foreach ($cfg['required'] as $req) {
        $val = $data[$req] ?? null;
        if ($val === null || (is_string($val) && trim($val)==='')) {
          echo json_encode(['success'=>false,'message'=>"Field wajib '$req' belum diisi."]); exit;
        }
      }
    }
    ...
    if ($action === 'store') {
      ...
      // binding dengan tipe null bila perlu
      foreach ($data as $k=>$v) {
        $param = is_null($v) ? PDO::PARAM_NULL : PDO::PARAM_STR;
        if (is_int($v)) $param = PDO::PARAM_INT;
  ...
  if ($action === 'delete') {
    ...
    try {
      ...
      if ($e->getCode()==='23000') {
        echo json_encode(['success'=>false,'message'=>'Tidak bisa menghapus: data sedang dipakai (foreign key).']); exit;
      }
      throw $e;
    }
  }
