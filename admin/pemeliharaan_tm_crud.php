<?php
// pages/pemeliharaan_tm_crud.php — LIST mendukung semua AFD (opsional filter AFD), CRUD + SweetAlert handled on client
session_start();
...
if ($ket!==''){ $where.=" AND ket LIKE :k"; $p[':k']="%$ket%"; }
  $sql="SELECT * FROM pemeliharaan_tm $where ORDER BY jenis_nama ASC, unit_kode ASC, id ASC";
  $st=$pdo->prepare($sql);
  ...
  $jenisMaster = $pdo->query("SELECT nama FROM md_pemeliharaan_tm ORDER BY nama")->fetchAll(PDO::FETCH_COLUMN);
...
try{
  if ($act==='store' || $act==='update'){
    ...
    $jenis_nm = $jenis_id ? getNamaById($pdo,'md_pemeliharaan_tm',$jenis_id,'nama') : null;
    ...
    if ($act==='store'){
      $sql="INSERT INTO pemeliharaan_tm
      ...
      $sql="UPDATE pemeliharaan_tm SET
  ...
  if ($act==='delete'){
    $id=(int)($_POST['id']??0);
    if ($id<=0) out(false,'ID tidak valid');
    $pdo->prepare("DELETE FROM pemeliharaan_tm WHERE id=:id")->execute([':id'=>$id]);
