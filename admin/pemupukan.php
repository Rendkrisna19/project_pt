...
try {
  ...
  } catch (Throwable $e) { $tahunTanamList = []; error_log("Error fetching master tahun tanam: " . $e->getMessage()); }

  // Fetch Master Baru (dengan error handling)
  try { $rayons = $conn->query("SELECT id, nama FROM md_rayon ORDER BY nama")->fetchAll(PDO::FETCH_ASSOC); } catch (Throwable $e) { $rayons = []; error_log("Error fetching master rayon: " . $e->getMessage()); }
  try { $apls = $conn->query("SELECT id, nama FROM md_apl ORDER BY nama")->fetchAll(PDO::FETCH_ASSOC); } catch (Throwable $e) { $apls = []; error_log("Error fetching master apl: " . $e->getMessage()); }
  try { $keterangans= $conn->query("SELECT id, keterangan FROM md_keterangan ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC); } catch (Throwable $e) { $keterangans = []; error_log("Error fetching master keterangan: " . $e->getMessage()); }
  try { $gudangs = $conn->query("SELECT id, nama FROM md_asal_gudang ORDER BY nama")->fetchAll(PDO::FETCH_ASSOC); } catch (Throwable $e) { $gudangs = []; error_log("Error fetching master gudang: " . $e->getMessage()); }
  // --------------------------------------------------
  ...
  $kebunIdToKode = []; foreach ($kebuns as $kb) { $kebunIdToKode[(int)$kb['id']] = $kb['kode']; }
  ...
  if ($tab === 'angkutan') {
    ...
    if($f_rayon_text!=='' && !$hasRayonIdA && $columnExists($conn,'angkutan_pupuk','rayon')){$where.=" AND a.rayon LIKE :ry";$p[':ry']="%$f_rayon_text%";}
    if($f_keterangan_text!=='' && !$hasKetIdA && $columnExists($conn,'angkutan_pupuk','keterangan')){$where.=" AND a.keterangan LIKE :ket";$p[':ket']="%$f_keterangan_text%";}
    ...
    elseif($hasTTValMenabur){$selectTT=", m.tahun_tanam AS t_tanam";$selectTTRaw=", m.tahun_tanam AS tahun_tanam_val";} // Ambil value tahun jika kolom ID tidak ada
    ...
    // Filter teks lama (jika kolom masih ada dan field ID tidak dipakai)
    if($f_rayon_text!=='' && !$hasRayonIdM && $columnExists($conn,'menabur_pupuk','rayon')){$where.=" AND m.rayon LIKE :ry";$p[':ry']="%$f_rayon_text%";}
    if($f_keterangan_text!=='' && !$hasKetIdM && $columnExists($conn,'menabur_pupuk','keterangan')){$where.=" AND m.keterangan LIKE :ket";$p[':ket']="%$f_keterangan_text%";}
    ...
    $sum_page_kg=0.0;$sum_page_luas=0.0;$sum_page_invt=0;
    foreach($rows as $r){$sum_page_kg+=(float)($r['jumlah']??0);$sum_page_luas+=(float)($r['luas']??0);$sum_page_invt+=(int)($r['invt_pokok']??0);}
  }
...
function qs_no_page(array $extra=[]){
  ...
  $base=[
      'tab'=>$_GET['tab']??'', 'unit_id'=>$_GET['unit_id']??'', 'kebun_id'=>$_GET['kebun_id']??'',
      'tanggal'=>$_GET['tanggal']??'', 'bulan'=>$_GET['bulan']??'', 'jenis_pupuk'=>$_GET['jenis_pupuk']??'',
      'rayon'=>$_GET['rayon']??'', 'keterangan'=>$_GET['keterangan']??'', // Keep old text filters?
      // Add new ID filters if they exist in GET
      'rayon_id'=>$_GET['rayon_id']??'', 'apl_id'=>$_GET['apl_id']??'',
      'keterangan_id'=>$_GET['keterangan_id']??'', 'gudang_asal_id'=>$_GET['gudang_asal_id']??''
  ];
...
<style>
  ...
  thead.sticky{position:sticky;top:0;z-index:10; background-color: #f9fafb;} /* Pastikan background thead */
  table.table-fixed{table-layout:fixed}
  button:disabled { opacity: 0.5; cursor: not-allowed !important; }
  .action-btn { background: none; border: none; padding: 0.25rem; cursor: pointer; border-radius: 0.25rem; }
  .action-btn:hover:not(:disabled) { background-color: rgba(0,0,0,0.05); }
  .action-btn:disabled svg { color: #9ca3af; /* gray-400 */ }
  .action-btn svg { width: 1.1rem; height: 1.1rem; }
  /* Style baris total */
  .summary-totals {
      background-color: #077d11ff; /* bg-gray-100 */ padding: 0.75rem 1rem; /* p-3 */
      margin-top: -1px; /* Rapatkan dengan tabel */ border: 1px solid #e5e7eb; border-top: none; /* Hilangkan border atas */
      border-bottom-left-radius: 0.75rem; border-bottom-right-radius: 0.75rem; /* rounded-b-xl */
      font-size: 0.875rem; /* text-sm */ color: #ffffffff; /* text-gray-800 */
      display: flex; justify-content: flex-end; /* Align to the right */ gap: 1.5rem; /* gap-6 */
  }
  .summary-totals strong { font-weight: 600; /* font-semibold */ }
</style>
  ...
  <div class="border-b border-gray-200 flex flex-wrap gap-2 md:gap-6">
    <?php
      $persist=[ // Persist filters
        'unit_id'=>$f_unit_id,'kebun_id'=>$f_kebun_id,'tanggal'=>$f_tanggal,'bulan'=>$f_bulan,
        'jenis_pupuk'=>$f_jenis,'per_page'=>$per_page,
        'rayon_id'=>$f_rayon_id, 'apl_id'=>$f_apl_id, 'keterangan_id'=>$f_keterangan_id, 'gudang_asal_id'=>$f_gudang_id, // New IDs
    ...
    <div class="mt-3 px-3 md:px-0 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
      ...
      <?php function page_link($p){
          $q=[
            'tab'=>$_GET['tab']??'', 'unit_id'=>$_GET['unit_id']??'', 'kebun_id'=>$_GET['kebun_id']??'',
            'tanggal'=>$_GET['tanggal']??'', 'bulan'=>$_GET['bulan']??'', 'jenis_pupuk'=>$_GET['jenis_pupuk']??'',
            'rayon_id'=>$_GET['rayon_id']??'', 'apl_id'=>$_GET['apl_id']??'', // Tambah filter ID baru
            'keterangan_id'=>$_GET['keterangan_id']??'', 'gudang_asal_id'=>$_GET['gudang_asal_id']??'',
            // 'rayon'=>$_GET['rayon']??'', 'keterangan'=>$_GET['keterangan']??'', // Teks lama opsional
            'per_page'=>$_GET['per_page']??'', 'page'=>$p,
          ];
    ...
    <form id="crud-form"><input type="hidden" name="csrf_token" value="<?=htmlspecialchars($CSRF)?>"><input type="hidden" name="action" id="form-action"><input type="hidden" name="id" id="form-id"><input type="hidden" name="tab" id="form-tab" value="<?=htmlspecialchars($tab)?>">
      ...
      <div id="group-angkutan" class="<?=$tab==='angkutan'?'':'hidden'?>">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><label class="block text-sm font-semibold mb-1">Kebun</label><select id="kebun_id_angkutan" name="kebun_id" class="i-select"><option value="">— Pilih Kebun —</option><?php foreach($kebuns as $k):?><option value="<?=(int)$k['id']?>" data-kode="<?=htmlspecialchars($k['kode'])?>"><?=htmlspecialchars($k['nama_kebun'])?> (<?=htmlspecialchars($k['kode'])?>)</option><?php endforeach;?></select><input type="hidden" id="kebun_kode_angkutan" name="kebun_kode"></div>
              <div><label class="block text-sm font-semibold mb-1">Rayon</label><select id="rayon_id_angkutan" name="rayon_id" class="i-select"><option value="">— Pilih Rayon —</option><?php foreach($rayons as $r):?><option value="<?=(int)$r['id']?>"><?=htmlspecialchars($r['nama'])?></option><?php endforeach;?></select></div>
              <div><label class="block text-sm font-semibold mb-1">Gudang Asal *</label><select id="gudang_asal_id" name="gudang_asal_id" required class="i-select"><option value="">— Pilih Gudang —</option><?php foreach($gudangs as $g):?><option value="<?=(int)$g['id']?>"><?=htmlspecialchars($g['nama'])?></option><?php endforeach;?></select></div>
              <div><label class="block text-sm font-semibold mb-1">Unit Tujuan *</label><select id="unit_tujuan_id" name="unit_tujuan_id" required class="i-select"><option value="">— Pilih Unit —</option><?php foreach($units as $u):?><option value="<?=(int)$u['id']?>"><?=htmlspecialchars($u['nama_unit'])?></option><?php endforeach;?></select></div>
              ...
              <div><label class="block text-sm font-semibold mb-1">Jenis Pupuk *</label><select id="jenis_pupuk_angkutan" name="jenis_pupuk" required class="i-select"><option value="">-- Pilih Jenis --</option><?php foreach($pupuks as $jp):?><option value="<?=htmlspecialchars($jp)?>"><?=htmlspecialchars($jp)?></option><?php endforeach;?></select></div>
              ...
              <div class="md:col-span-2"><label class="block text-sm font-semibold mb-1">Keterangan</label><select id="keterangan_id_angkutan" name="keterangan_id" class="i-select"><option value="">— Pilih Keterangan —</option><?php foreach($keterangans as $k):?><option value="<?=(int)$k['id']?>"><?=htmlspecialchars($k['keterangan'])?></option><?php endforeach;?></select></div>
      ...
      <div id="group-menabur" class="<?=$tab==='menabur'?'':'hidden'?>">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div class="md:col-span-1"><label class="block text-sm font-semibold mb-1">Unit *</label><select id="unit_id" name="unit_id" required class="i-select"><option value="">— Pilih Unit —</option><?php foreach($units as $u):?><option value="<?=(int)$u['id']?>"><?=htmlspecialchars($u['nama_unit'])?></option><?php endforeach;?></select></div>
              ...
              <div class="md:col-span-1"><label class="block text-sm font-semibold mb-1">Blok *</label><select id="blok" name="blok" required class="i-select"><option value="">— pilih Unit dulu —</option></select></div>
              <div><label class="block text-sm font-semibold mb-1">Rayon</label><select id="rayon_id_menabur" name="rayon_id" class="i-select"><option value="">— Pilih Rayon —</option><?php foreach($rayons as $r):?><option value="<?=(int)$r['id']?>"><?=htmlspecialchars($r['nama'])?></option><?php endforeach;?></select></div>
              ...
              <div><label class="block text-sm font-semibold mb-1">Jenis Pupuk *</label><select id="jenis_pupuk_menabur" name="jenis_pupuk" required class="i-select"><option value="">-- Pilih Jenis --</option><?php foreach($pupuks as $jp):?><option value="<?=htmlspecialchars($jp)?>"><?=htmlspecialchars($jp)?></option><?php endforeach;?></select></div>
              <div><label class="block text-sm font-semibold mb-1">Tahun Tanam</label><select id="tahun_tanam_select" name="tahun_tanam_dummy" class="i-select"><option value="">— Pilih Tahun Tanam —</option><?php foreach(($tahunTanamList??[])as $tt):?><option value="<?=htmlspecialchars($tt['tahun'])?>" data-id="<?=(int)$tt['id']?>"><?=htmlspecialchars($tt['tahun'])?><?=$tt['ket']?' - '.htmlspecialchars($tt['ket']):''?></option><?php endforeach;?></select><input type="hidden" name="tahun_tanam_id" id="tahun_tanam_id"><input type="hidden" name="tahun_tanam" id="tahun_tanam_val"></div>
              <div><label class="block text-sm font-semibold mb-1">APL (Aplikasi)</label><select id="apl_id" name="apl_id" class="i-select"><option value="">— Pilih APL —</option><?php foreach($apls as $a):?><option value="<?=(int)$a['id']?>"><?=htmlspecialchars($a['nama'])?></option><?php endforeach;?></select></div>
              ...
              <div class="md:col-span-2"><label class="block text-sm font-semibold mb-1">Keterangan</label><select id="keterangan_id_menabur" name="keterangan_id" class="i-select"><option value="">— Pilih Keterangan —</option><?php foreach($keterangans as $k):?><option value="<?=(int)$k['id']?>"><?=htmlspecialchars($k['keterangan'])?></option><?php endforeach;?></select></div>
          </div>
      </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadJSON(url){try{const r=await fetch(url);if(!r.ok) throw new Error(r.statusText);return await r.json()}catch(e){console.error("Fetch failed:",url,e);return null}}
  ...
  async function refreshFormBlokByUnit(){const uid=$('#unit_id')?.value||'';const sel=$('#blok');if(!sel)return;sel.disabled=!uid;if(!uid){sel.innerHTML='<option value="">— pilih Unit dulu —</option>';return}sel.innerHTML='<option value="">Memuat...</option>';const j=await loadJSON(`?ajax=options&type=blok&unit_id=${encodeURIComponent(uid)}`);const list=(j&&j.success&&Array.isArray(j.data))?j.data:[];fillSelect(sel,list,'-- Pilih Blok --')}
  ...
  if (!IS_STAF && $('#btn-add')) {
    $('#btn-add').addEventListener('click',()=>{
        ...
        if(currentTab==='menabur'){ refreshFormBlokByUnit(); ttSel.dispatchEvent(new Event('change')); } // Reset & trigger TT sync
        if(currentTab==='angkutan'){$('#kebun_kode_angkutan').value=''}
        open()
    });
  }

  $('#btn-close')?.addEventListener('click',close);$('#btn-cancel')?.addEventListener('click',close);
  document.addEventListener('keydown', (e)=>{ if(e.key==='Escape' && !modal.classList.contains('hidden')) close(); }); // Tutup modal dengan Esc
  ...
  document.body.addEventListener('click',async e=>{
    ...
    if(btn.classList.contains('btn-delete') && !IS_STAF){
      ...
      Swal.fire({title:'Hapus data ini?',text:'Tindakan ini tidak dapat dibatalkan.',icon:'warning',showCancelButton:true, confirmButtonColor: '#dc2626', confirmButtonText: 'Ya, Hapus', cancelButtonText: 'Batal'}).then(res=>{
        if(!res.isConfirmed)return;
        const fd=new FormData();fd.append('csrf_token','<?=htmlspecialchars($CSRF)?>');fd.append('action','delete');fd.append('tab',tab);fd.append('id',id);
        fetch('pemupukan_crud.php',{method:'POST',body:fd})
          .then(r => {
              if (!r.ok) { throw new Error(`HTTP error! status: ${r.status}`); }
              return r.json();
          })
          .then(j=>{if(j.success)Swal.fire({icon:'success',title:'Terhapus',timer:1200,showConfirmButton:false}).then(()=>location.reload());else Swal.fire('Gagal',j.message||'Error saat menghapus','error')})
  ...
  if (!IS_STAF && form) {
    form.addEventListener('submit',e=>{
      ...
      fetch('pemupukan_crud.php',{method:'POST',body:fd})
        ...
        .then(j=>{if(j.success){close();Swal.fire({icon:'success',title:'Berhasil',text:j.message||'Tersimpan',timer:1400,showConfirmButton:false}).then(()=>location.reload())}else{const html=j.errors?.length?`<ul class="list-disc list-inside text-left">${j.errors.map(e=>`<li>${e}</li>`).join('')}</ul>`:(j.message||'Terjadi kesalahan');Swal.fire('Gagal',html,'error')}})
        .catch(err=>Swal.fire('Error',`Gagal menghubungi server: ${err.message}`,'error'));
    });
  } // End if(!IS_STAF && form)
