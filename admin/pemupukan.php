...
<?php endif; // End if fetch_table ?>
    ...
    <div class="sticky-container">
        <table class="table-grid">
            <thead>
                <tr>
                    <?php if ($tab==='angkutan'): ?>
                        ...
                        <?php if ($canAction): ?><th style="text-align:center; min-width:80px">Aksi</th><?php endif; ?>
                    <?php else: ?>
            ...
            <tbody>
                <?php if (empty($rows)): ?>
                    <tr><td colspan="<?= ($tab==='angkutan' ? ($canAction?10:9) : ($canAction?17:16)) ?>" class="text-center py-8 italic text-gray-500">Data tidak ditemukan.</td></tr>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (CAN_INPUT) {
        ...
        document.querySelector('#table-wrapper').addEventListener('click', async e => {
            ...
            // EDIT: Hanya jika CAN_ACTION (Admin)
            if(btnE && CAN_ACTION){
            ...
            // DELETE: Hanya jika CAN_ACTION (Admin)
            if(btnD && CAN_ACTION){
                ...
                .then(res=>{
                    ...
                    .then(r=>r.json()).then(j=>{
                        if(j.success) Swal.fire('Terhapus','','success').then(()=> fetchData($('#current_page').value)); 
        ...
        form.addEventListener('submit', e=>{
            ...
            .then(r=>r.json()).then(j=>{
                if(j.success){ 
                    ...
                    .then(()=> fetchData($('#current_page').value));
                }
                else Swal.fire('Gagal', j.message||'Error', 'error');
            });
        });
